export default function FinalReport({ session, onRestart }) {
  const scores = [
    { label: 'Overall',    value: session.overallScore,       bar: '#22c55e' },
    { label: 'Comms',      value: session.communicationScore, bar: '#84cc16' },
    { label: 'Confidence', value: session.confidenceScore,    bar: '#a3e635' },
    { label: 'Technical',  value: session.technicalScore,     bar: '#4ade80' },
  ];
  const readiness = session.readinessPercentage ?? 0;

  return (
    <div className="min-h-screen bg-[#080e0a] flex flex-col items-center justify-start py-10 px-4 overflow-y-auto font-mono">

      <div className="w-full max-w-3xl text-center mb-6">
        <div className="inline-flex items-center gap-2 px-3 py-1 rounded bg-[#22c55e0f] border border-[#22c55e33] text-[#22c55e] text-[10px] tracking-widest uppercase mb-4">
          <span className="w-1.5 h-1.5 rounded-full bg-[#22c55e] recording-dot" />
          Interview Complete
        </div>
        <h1 className="text-2xl font-bold text-[#e8f5ea] mb-1 font-sans">Your Interview Report</h1>
        <p className="text-[11px] text-[#2d6e3a] tracking-wider">
          <span className="text-[#4a8f54]">{session.candidateName}</span>{' · '}
          <span className="text-[#4a8f54]">{session.jobRole}</span>{' · '}
      ...
      <div className="flex flex-col items-center mb-6">
        ...
        <p className="text-[10px] tracking-widest uppercase text-[#2d6e3a] mt-2">Interview Readiness</p>
      </div>

      <div className="w-full max-w-3xl grid grid-cols-2 md:grid-cols-4 gap-2 mb-5">
        {scores.map((s) => <ScoreCard key={s.label} {...s} />)}
      </div>

      {session.overallFeedback && (
        <div className="w-full max-w-3xl bg-[#0c1a0e] border border-[#1a3a20] rounded-lg p-4 mb-4">
      ...
      {session.recommendations && (
        <div className="w-full max-w-3xl bg-[#0c1a0e] border border-[#38bdf833] rounded-lg p-4 mb-6">
          ...
          <ul className="space-y-2">
            {session.recommendations.split(/[.\n]/).filter((r) => r.trim().length > 5).map((r, i) => (
              <li key={i} className="flex gap-2 text-[11px] text-[#8fbf96] font-sans leading-relaxed">
                <span className="text-[#38bdf8] shrink-0 text-[10px]">{String(i + 1).padStart(2, '0')}.</span>
                {r.trim()}
              </li>
            ))}
          </ul>
...
function ScoreCard({ label, value, bar }) {
  const score = value ?? 0;
  return (
    <div className="relative bg-[#0c1a0e] border border-[#1a3a20] rounded-lg p-3 text-center overflow-hidden">
      ...
      <p className="text-3xl font-bold leading-none mb-2" style={{ color: bar }}>{score}</p>
      <div className="h-0.5 rounded-full bg-[#1a3a20] overflow-hidden">
        <div className="h-full rounded-full transition-all duration-1000" style={{ width: `${score}%`, background: bar }} />
      </div>
    </div>
  );
}
...
function ReadinessRing({ value }) {
  ...
  return (
    <svg width="120" height="120" viewBox="0 0 140 140">
      ...
      <text x="70" y="65" textAnchor="middle" fill={color} fontSize="22" fontWeight="700" fontFamily="JetBrains Mono, monospace">{value}%</text>
      <text x="70" y="83" textAnchor="middle" fill="#2d6e3a" fontSize="9" fontFamily="JetBrains Mono, monospace" letterSpacing="2">{label}</text>
    </svg>
  );
}
